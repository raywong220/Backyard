...
class WeightedEdge(Edge):
    def __init__(self, src, dest, total_distance, outdoor_distance):
        # assert all(isinstance(src, Node) and isinstance(dest, Node))
        # assert all(
        #     isinstance(total_distance, int) and isinstance(outdoor_distance, int)
        # )
    ...
    def __str__(self):
        return "{}->{} ({}, {})".format(
            self.src, self.dest, self.total_distance, self.outdoor_distance
        )
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        if not (
            self.has_node(edge.get_source()) and self.has_node(edge.get_destination())
        ):
            raise ValueError("Either of the nodes is not in the graph.")
