...
class Patient(object):
    ...
    def update(self):
        ...
        """
        for index, bacteria in enumerate(self.bacteria):
            if bacteria.is_killed():
                self.bacteria[index] = None
        self.bacteria = list(filter(None, self.bacteria))
...
def calc_pop_avg(populations, n):
    ...
    """
    pop_avg = list(zip(*populations))[n]
    return sum(pop_avg) / len(pop_avg)
...
def simulation_without_antibiotic(
    ...
    for trial in range(num_trials):
        start_bact = [SimpleBacteria(birth_prob, death_prob)] * num_bacteria
        patient = Patient(start_bact, max_pop)
        populations.append([num_bacteria])
        for timestep in range(299):
            populations[trial].append(patient.update())

    avg_populations = [calc_pop_avg(populations, n) for n in range(300)]
    make_one_curve_plot(
        list(range(300)),
        avg_populations,
        "Timestep",
...
def calc_pop_std(populations, t):
    ...
    for i in range(len(populations)):
        diff.append((populations[i][t] - mean) ** 2)
    return (sum(diff) / len(populations)) ** 0.5
    ...
    def reproduce(self, pop_density):
        ...
        if self.birth_prob * (1 - pop_density) >= random.random():
            return ResistantBacteria(
                ...
                else self.mut_prob * (1 - pop_density) >= random.random(),
                self.mut_prob,
            )
        raise NoChildException
...
class TreatedPatient(Patient):
    ...
    def get_resist_pop(self):
        ...
        """
        return len([bact for bact in self.bacteria if bact.get_resistant()])
    ...
    def update(self):
        ...
        for index, bacteria in enumerate(self.bacteria):
            if bacteria.is_killed():
                self.bacteria[index] = None
            elif self.on_antibiotic and not bacteria.resistant:
                self.bacteria[index] = None
        self.bacteria = list(filter(None, self.bacteria))
        pop_density = len(self.bacteria) / self.max_pop
        offspring = []
...
def simulation_with_antibiotic(
    ...
    for trial in range(num_trials):
        ...
        patient = TreatedPatient(start_bact, max_pop)
        populations.append([num_bacteria])
        resistant_pop.append([0])

        for timestep in range(400):
            if timestep == 150:
                patient.on_antibiotic = True
            populations[trial].append(patient.update())
            resistant_pop[trial].append(patient.get_resist_pop())

    avg_populations = [calc_pop_avg(populations, n) for n in range(400)]
