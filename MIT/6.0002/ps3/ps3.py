...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.direction = random.uniform(0, 360) % 360  # In case it draws 360, %
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        cur_angle = self.get_robot_direction()
        new_pos = cur_pos.get_new_position(cur_angle, self.speed)
        if self.room.is_position_valid(new_pos):
            ...
            self.set_robot_direction(random.uniform(0, 360) % 360)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials, robot_type):
    ...
    def one_sim(room, robots, min_coverage):
        time_steps = 0
    ...
    for trial in range(num_trials):
        ...
        timesteps = one_sim(room, robots, min_coverage)
        each_mean.append(timesteps)

    return sum(each_mean) / num_trials
...
#       of a 20x20 room?
#
#       Both StandardRobot and FaultyRobot have similar time improvement (trend) as the
#       number of robots used increases (first steeply, then gradually decline).
#       FaultyRobot took more steps than StandardRobot in all tests.
#       However, the differences between them decline as the number of robots used increases.
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions
#       10x30, 20x15, 25x12, and 50x6?
#
#       In general, the performance of StandardRobot is better than that of FaultyRobot and
#       the time needed gradually rises when the width-to-height ratio of the room increases in both robots.
#       Except their time used to clean 10x30 (ratio: 0.333) is slightly longer than
#       time used to clean 20x15 (ratio: 1.33) and 25x12 (ratio: 2.08).
#
...
e_room = EmptyRoom(10, 10, 2)
robots = [StandardRobot(e_room, 1, 1) for i in range(2)]
animation(e_room, robots, 0.8)
